import React from 'react';
import { Metadata } from 'next';
import { HeartPulse, Dumbbell, ShieldCheck, Stethoscope, Trophy, CheckCircle } from 'lucide-react';
import ContactForm from '@/components/ContactForm';
import { siteConfig } from "@/lib/config";
import { ContactLink } from "@/components/custom/contact-link";
import Image from 'next/image';

export const metadata: Metadata = {
  title: 'Medicina Desportiva | MediSigma',
  description: 'Serviços de Medicina Desportiva para atletas e clubes. Avaliações de aptidão, prevenção de lesões e otimização de performance.',
  keywords: 'medicina desportiva, aptidão desportiva, prevenção de lesões, performance, atletas, MediSigma',
  alternates: {
    canonical: 'https://www.medisigma.pt/servicos/medicina-desportiva/',
  },
};

const jsonLd = {
  '@context': 'https://schema.org',
  '@type': 'Service',
  'name': 'Medicina Desportiva',
  'provider': {
    '@type': 'Organization',
    'name': 'MediSigma',
    'url': 'https://medisigma.pt',
  },
  'serviceType': 'Medicina Desportiva',
  'description': 'Serviços de Medicina Desportiva para atletas e clubes, focados na prevenção, diagnóstico e tratamento de lesões, e otimização da performance.',
};

export default function MedicinaDesportivaPage() {
  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}
      />

      <main className="min-h-screen bg-white flex flex-col divide-y divide-border">
        {/* Hero Section */}
        <section className="relative py-16 md:py-24 mx-4 md:mx-8 rounded-3xl mb-8">
          <div className="absolute inset-0 -z-10 pointer-events-none">
            <div className="animated-hero-background absolute inset-0 -z-10 h-full w-full [background:radial-gradient(125%_125%_at_50%_10%,var(--background)_40%,var(--secondary)_100%)] rounded-3xl"></div>
          </div>
          <div className="container mx-auto px-6 md:px-8 max-w-6xl">
            <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center">
              <div>
                <h1 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6 leading-tight">
                  Medicina Desportiva
                </h1>
                <p className="text-lg md:text-xl text-gray-700 mb-8 leading-relaxed">
                  O nosso serviço de Medicina Desportiva é dedicado a atletas de todos os níveis. Realizamos exames de aptidão, prevenimos lesões e otimizamos a performance para garantir que atinge os seus objetivos em segurança.
                </p>
                <div className="flex flex-col sm:flex-row gap-4">
                  <a
                    href="#cta-section"
                    className="bg-secondary text-primary-foreground px-8 py-3 rounded-lg font-semibold hover:bg-secondary/90 transition-colors text-center cursor-pointer"
                  >
                    Fale connosco
                  </a>
                </div>
              </div>
              <div className="text-center">
                {/* Imagem Placeholder - substituir pela imagem correta */}
                <Image src="/images/uploads/Design sem nome (20).jpg" alt="Medicina Desportiva" width={500} height={500} className="max-w-full h-auto rounded-lg shadow-lg" />
              </div>
            </div>
          </div>
        </section>

        {/* What is Sports Medicine Section */}
        <section className="py-16 md:py-20 mx-4 md:mx-0">
          <div className="container mx-auto px-6 md:px-8 max-w-6xl">
            <div className="text-center mb-12 md:mb-16">
              <h2 className="text-2xl md:text-4xl font-bold text-gray-900 mb-4 md:mb-6">
                O que é a Medicina Desportiva?
              </h2>
              <p className="text-base md:text-lg text-gray-600 max-w-4xl mx-auto leading-relaxed">
                A Medicina Desportiva foca-se na saúde e bem-estar de atletas. Através de avaliações rigorosas, garantimos a aptidão para a prática desportiva, prevenimos lesões e apoiamos a recuperação, maximizando o potencial de cada atleta.
              </p>
            </div>
          </div>
        </section>

        {/* Services Section */}
        <section className="bg-accent py-16 md:py-20 mx-4 md:mx-8 rounded-3xl mb-8">
          <div className="container mx-auto px-6 md:px-8 max-w-6xl">
            <div className="text-center mb-12 md:mb-16">
              <h2 className="text-2xl md:text-4xl font-bold text-gray-900 mb-4 md:mb-6">
                Os Nossos Serviços
              </h2>
              <p className="text-base md:text-lg text-gray-600 max-w-3xl mx-auto">
                Disponibilizamos um conjunto de serviços essenciais para atletas, clubes e federações.
              </p>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {[
                { icon: Stethoscope, title: "Exames de Aptidão Desportiva", description: "Avaliações médico-desportivas completas para garantir a aptidão do atleta." },
                { icon: ShieldCheck, title: "Prevenção de Lesões", description: "Programas personalizados para minimizar o risco de lesões." },
                { icon: HeartPulse, title: "Otimização de Performance", description: "Aconselhamento para melhorar o rendimento físico e a recuperação." },
              ].map((service) => {
                const Icon = service.icon;
                return (
                  <div key={service.title} className="bg-white p-6 md:p-8 rounded-xl shadow-sm border border-border text-center">
                    <div className="w-16 h-16 bg-secondary/10 rounded-lg flex items-center justify-center mx-auto mb-6">
                      <Icon className="w-8 h-8 text-secondary" />
                    </div>
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">{service.title}</h3>
                    <p className="text-muted-foreground">{service.description}</p>
                  </div>
                );
              })}
            </div>
          </div>
        </section>

        {/* Benefits Section */}
        <section className="py-16 md:py-20 mx-4 md:mx-0">
          <div className="container mx-auto px-6 md:px-8 max-w-6xl">
            <div className="text-center mb-12 md:mb-16">
              <h2 className="text-2xl md:text-4xl font-bold text-gray-900 mb-4 md:mb-6">
                Benefícios da Medicina Desportiva
              </h2>
              <p className="text-base md:text-lg text-gray-600 max-w-3xl mx-auto">
                Investir no acompanhamento médico-desportivo é essencial para uma carreira longa e bem-sucedida.
              </p>
            </div>
            <div className="space-y-4">
              {[
                { icon: Trophy, text: "Melhoria do Desempenho Atlético" },
                { icon: ShieldCheck, text: "Redução do Risco de Lesões" },
                { icon: HeartPulse, text: "Promoção de um Estilo de Vida Saudável" },
                { icon: Dumbbell, text: "Recuperação Mais Rápida e Eficaz" }
              ].map(point => {
                const Icon = point.icon;
                return (
                  <div key={point.text} className="bg-white p-4 rounded-lg flex items-center gap-4 border border-border shadow-sm">
                    <div className="w-10 h-10 bg-secondary/10 rounded-lg flex items-center justify-center flex-shrink-0">
                      <Icon className="w-5 h-5 text-secondary" />
                    </div>
                    <span className="font-semibold">{point.text}</span>
                  </div>
                )
              })}
            </div>
          </div>
        </section>

        {/* CTA Section */}
        <section id="cta-section" className="relative z-10 bg-secondary py-16 md:py-20 mx-4 md:mx-8 rounded-3xl mb-8">
          <div className="container mx-auto px-6 md:px-8 max-w-5xl">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div className="text-white">
                <h2 className="text-2xl md:text-4xl font-bold mb-4 md:mb-6">
                  Maximize o Potencial dos Seus Atletas
                </h2>
                <p className="text-base md:text-lg text-primary-foreground/90 max-w-3xl mx-auto mb-8">
                  Oferecemos um acompanhamento especializado para garantir que os seus atletas competem ao mais alto nível, com total segurança.
                </p>

                <div className="space-y-4 mb-8 text-blue-100">
                  <div className="flex items-center space-x-3">
                    <CheckCircle className="w-5 h-5 text-white" />
                    <span>Exames de aptidão para todas as modalidades</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <CheckCircle className="w-5 h-5 text-white" />
                    <span>Programas de prevenção de lesões</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <CheckCircle className="w-5 h-5 text-white" />
                    <span>Aconselhamento para otimização de performance</span>
                  </div>
                </div>

                <div className="flex flex-col sm:flex-row gap-4">
                  <a
                    href={`https://wa.me/${siteConfig.links.whatsapp}?text=Olá%2C%20estava%20no%20vosso%20website%20e%20gostaria%20de%20ter%20mais%20informações%20sobre%20Medicina%20Desportiva.`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-white text-secondary px-8 py-3 rounded-lg font-semibold hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.515z" />
                    </svg>
                    WhatsApp
                  </a>
                  <ContactLink href="tel:241331504" type="phone" pagina="Serviço Medicina Desportiva" className="border-2 border-white text-white px-8 py-3 rounded-lg font-semibold hover:bg-white hover:text-secondary transition-colors text-center">
                    241 331 504
                  </ContactLink>
                </div>
              </div>

              <ContactForm
                pagina="Serviço Medicina Desportiva"
                fonte="servicos/medicina-desportiva"
                servicoDefault="Medicina Desportiva"
              />
            </div>
          </div>
        </section>
      </main>
    </>
  );
}


