  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (cvMode === 'file' && cvFile) {
        ...
        formData.append('area_interesse', state.area_interesse || 'Candidatura Espontânea');
        formData.append('mensagem', state.mensagem || '');
        formData.append('pagina', 'Candidatura Espontânea');
        formData.append('url', typeof window !== 'undefined' ? window.location.href : '');
        formData.append('cv', cvFile);
        body = formData;
        // Headers deixados vazios para o browser definir Content-Type multipart/form-data com boundary
      } else {
        body = JSON.stringify({
          nome,
          email,
